...
(function () {
  ...
  /* ---------- Modale tarificateur : même voile de chargement / erreur ---------- */
  /* L'iframe de la modale reçoit son src à l'ouverture ; on affiche le voile à
     ce moment-là, masqué au `load`, avec repli d'erreur après délai. */
  (function initModalFrameLoader() {
    var modalEl = document.getElementById('modal-souscription');
    if (!modalEl) return;
    var loader = modalEl.querySelector('.modal-loading');
    var iframe = modalEl.querySelector('iframe');
    if (!loader || !iframe) return;
    var spinnerHtml = loader.innerHTML;
    var settled = false, timer = null, wired = false;
    function currentSrc() { return iframe.getAttribute('src') || iframe.getAttribute('data-src') || ''; }
    function done() { settled = true; if (timer) { clearTimeout(timer); timer = null; } }
    function hide() { if (settled) return; done(); loader.classList.add('is-hidden'); }
    function startTimer() { if (timer) clearTimeout(timer); timer = setTimeout(showError, FRAME_TIMEOUT); }
    function showError() {
      if (settled) return; done();
      loader.classList.remove('is-hidden');
      loader.classList.add('is-error');
      loader.innerHTML =
        ...
        '<p class="frame-error-actions">' +
        '<button type="button" class="btn frame-retry">Réessayer</button> ' +
        '<a class="frame-newtab" target="_blank" rel="noopener" href="' + currentSrc() + '">Ouvrir dans un nouvel onglet ↗</a>' +
        '</p>';
      var retry = loader.querySelector('.frame-retry');
      if (retry) retry.addEventListener('click', function () {
        var src = currentSrc();
    ...
    /* Démarrer le voile quand la modale s'ouvre (classe .show ajoutée par openModal). */
    var obs = new MutationObserver(function () {
      if (modalEl.classList.contains('show') && !wired) { wireOnce(); startTimer(); }
    });
