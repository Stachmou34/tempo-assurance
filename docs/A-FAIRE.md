...
- 🔴 **BUG JL Assure `date_debut` → 500** (2026-07-01) : `date_debut` en GET fait planter le tarificateur (page blanche), `mb.php` ET `mb43.php`. **Contournement en place** : `date_debut`/`heure_debut` retirés du relais partout (site + app). **À faire** : envoyer `docs/message-jlassure-date-debut-500.md` à christophe.p@jlassure.com, puis **réactiver** `date_debut` une fois corrigé.
