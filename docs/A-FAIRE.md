...
- **Phase 2bis (transmission des pièces à JL Assure)** : objectif zéro re-téléversement — les photos permis/carte grise partagées dans ChatGPT seraient jointes au dossier via API. Côté OpenAI c'est possible (`openai/fileParams` : notre serveur reçoit une URL de téléchargement). **Manque l'endpoint JL Assure** → envoyer `docs/message-jlassure-upload-pieces.md` à christophe.p@jlassure.com ; j'implémente dès leur réponse (spec connue). ⚠️ Élargit le périmètre RGPD (transit de pièces d'identité par notre serveur) → à ajouter au registre + politique de confidentialité au moment de l'implémentation. Caveat connu : bug OpenAI sur les uploads depuis mobile (références incomplètes) à re-tester.
